...
export default function Groups() {
	...
	const columns = [
		{
			title: t('group.name.field'),
			dataIndex: 'name',
			key: 'name',
		},
		{
			title: t('group.nickname.field'),
			dataIndex: 'nickname',
			key: 'nickname',
		},
		{
			title: t('group.description.field'),
			dataIndex: 'description',
			key: 'description',
		},
		{
			title: t('group.leader.field'),
			dataIndex: 'leader',
			key: 'leader',
	...
	async function getModalData(id) {
		...
		if (data.manager) data.manager = data.manager.id;
		setModalData(data);
	...
	async function deleteData(id) {
		try {
			...
			message.success(t('group.successfully-deleted.message'));
		} catch (error) {
	...
	return (
		<>
			...
			>
				<Typography.Title level={3}>{t('group.title')}</Typography.Title>
			</Layout.Content>
