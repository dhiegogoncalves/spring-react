...
export default function CreateUpdateForm({
	...
	const handleCreate = async fields => {
		try {
			...
			message.success(t('group.successfully-created.message'));
			return true;
		} catch (error) {
	...
	const handleUpdate = async fields => {
		try {
			await api.put('groups', fields);
			message.success(t('group.successfully-updated.message'));
			return true;
		} catch (error) {
			message.error(t('error.message'));
			return false;
		...
		<>
			<Form id='role_form' form={form} initialValues={data} onFinish={onFinish}>
				<Modal
					...
					title={
						data === null
							? t('group.modal-new.title')
							: t('group.modal-update.title')
					}
